...
export function App() {
  ...
  const nav: { id: AppPage; icon: string; label: string }[] = [
    { id: 'analysis', icon: '♟', label: 'Analysis' }, { id: 'library', icon: '▦', label: 'Library' },
    { id: 'play', icon: '▷', label: 'Play' },
    { id: 'settings', icon: '⚙', label: 'Settings' }, { id: 'licenses', icon: 'ⓘ', label: 'Licenses' },
  ]

  return <div className="app-shell">
    <nav className="navigation" aria-label="Main navigation"><button className="brand" onClick={() => void navigate('library')} aria-label="ChessIn home"><span className="brand-mark">♞</span><span className="brand-name">chessin<span className="brand-dot">.</span></span></button><div className="nav-items">{nav.map(item => <button key={item.id} onClick={() => void navigate(item.id)} className={page === item.id ? 'active' : ''} aria-current={page === item.id ? 'page' : undefined}><span className="nav-icon" aria-hidden="true">{item.icon}</span><span className="nav-label">{item.label}</span></button>)}</div><span className="nav-footer" title="Analysis stays on this device">LOCAL FIRST</span></nav>
    <main className="main-content">
      <div className="topbar"><span className="mobile-brand">♞ chessin<span>.</span></span><span className="topbar-path">WORKSPACE <span>/</span> {page.toUpperCase()}</span><div className="topbar-right">{page !== 'play' && <span className={`save-indicator ${saveState}`} role="status"><span className="status-dot" />{saveState === 'saving' ? 'Saving…' : saveState === 'unsaved' ? 'Unsaved' : 'Saved locally'}</span>}<span className="provider-badge">{engine.provider === 'remote' ? '↗ Remote' : '◉ Local'} · {engine.descriptor?.name || engine.status}</span></div></div>
      ...
      {page === 'settings' && <section className="utility-page"><span className="eyebrow">YOUR WORKSPACE</span><h1>Settings</h1><p>ChessIn stores games and preferences on this device. Remote analysis connects only when you explicitly choose it.</p><div className="settings-card"><h2>Engine & connection</h2><EngineSettingsPanel /></div></section>}
