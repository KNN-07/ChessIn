...
- **Analyze without an account.** A board-first workspace keeps ChessIn’s muted green panels and compact typography, with selected-move feedback, color-coded figurine notation, on-board quality badges and an evaluation timeline beside the board. First/previous/play-pause/next/last controls sit below the graph, with Moves, Engine and Review tabs preserving the full analysis workflow. Player ratings and recorded PGN clocks appear when present; missing scores and clocks are never fabricated. Import PGN variations/comments, paste FEN and export your annotated game; your library stays in your browser.
...
Open **Position tools & game quality → Engine** for quick analysis settings: depth/time presets, continuous search and the engine's supported line count. The drawer closes with Escape. Downloads, hardware limits, connection credentials and app installation remain on the full Settings page, reached through the header's gear or **Open full Settings**. Quick analysis controls are disabled in Review, which has its own search settings. The workspace stacks the board above the review panel on narrow screens without a fixed engine rail covering either surface.
