...
- **Analyze without an account.** A board-first workspace uses flat, muted green surfaces with selected-move feedback, color-coded figurine notation, on-board quality badges and an evaluation timeline beside the board. One sidebar keeps the move list visible: a large **Analyze game** button and progress bar replace the duplicate Moves/Engine/Review tabs. First/previous/play-pause/next/last controls sit below the graph. Player ratings and recorded PGN clocks appear when present; missing scores and clocks are never fabricated. Import PGN variations/comments, paste FEN and export your annotated game; your library stays in your browser.
- **Review with evidence.** Analyze, pause and resume from one primary control. Select a move for its classification, explanation and achieved depth; **Hint** shows its best engine continuation. White/Black **ChessIn quality** remains under **Position tools & game quality**. A **Brilliant** move uses an explicitly labeled *engine-backed sacrifice heuristic*, not a claim of perfect chess understanding. Incomplete or bounded comparisons remain **Uncertain**; ChessIn quality is not Chess.com accuracy or an Elo rating.
  Move-quality badges share a Chess.com-inspired circular SVG treatment across the board, notation and review panels: teal `!!`, blue `!`, green star/thumbs-up/check, and yellow/orange/red error symbols. Classification labels remain available as text or accessible names; color is not the only cue.
- **Recognize openings and understand moves.** An offline, curated set of common opening landmarks identifies the deepest recognized position on the selected variation, including transpositions, and shows its name and ECO code. This is not a complete opening database; unknown lines remain unrecognized, and PGN opening headers are not treated as detection. Move comments describe observable chess facts such as development, captures, castling, central-square attacks, promotion and check. Imported comments and engine assessments remain separate; generated feedback does not overwrite PGN annotations.
- **Play at your pace.** Analysis, new-game setup and ongoing games share the same board geometry, player rows, clocks, panel headers and responsive layout. Choose your color and time-control cards, and switch between New game, Engine and Rules before starting. Casual games have real clocks, paired notation and a Game details tab; move entry and game actions stay in the side panel. Completed games open directly in review. Mode-specific controls stay separate: no evaluations, analysis arrows or disguised engine hints during play.
...
Drag a piece, tap source and target, or enter SAN/UCI under **Position tools & game quality**. Both pointer methods offer queen, rook, bishop and knight promotion. Arrow keys navigate moves outside form controls; playback advances one move about every second and stops at the end of the selected line. A different continuation creates a variation, which you can promote to the main line. Deleting a branch or game requires confirmation. The header's flip button rotates the board/bar without changing White-positive score meaning. **Share position** copies the selected FEN; the hint button reveals a reviewed best continuation when available.
...
Open **Engine** directly beneath the right sidebar heading for quick analysis settings: depth/time presets, continuous search and the engine's supported line count. Settings expand inline and close with Escape. Downloads, hardware limits, connection credentials and app installation remain on the full Settings page, reached through the header's gear or **Open full Settings**. Quick controls and **Live engine analysis** are disabled while game analysis runs; pause to change preferences. Review depth/time remain under **Analysis settings & method**. The workspace stacks the board above the sidebar on narrow screens without a fixed engine rail covering either surface.
...
**Ready offline** requires both a controlled app shell and a verified, complete JS/WASM cache pair. First installation takes control of the current page without reloading. After an explicit download, the selected local engine initializes automatically; an already-installed engine also starts on later visits or when selected. Initialization is shared with Analyze/Play/Review requests, so they wait for the same worker instead of restarting it. Startup failures offer explicit Retry rather than retrying silently. Downloads and remote connections remain opt-in, and app-version updates still wait for an idle, saved state before reloading. Browser eviction may require another download; no games are deleted to make engine space. Install through the browser prompt when available, or Safari's Share → Add to Home Screen.

**App updates:** a normal browser reload can still open the cached release while an update is waiting. Use **Settings → App → Apply app update when idle** to activate the new release, including layout fixes. This is separate from engine initialization and does not require deleting imported games or downloaded engines.
...
Review compares exact, completed MultiPV snapshots at the greatest common depth. If the played move is absent from the deepest completed comparison—even if it appeared in an earlier iteration—ChessIn performs a restricted search from the same pre-move position. Later bounded updates do not overwrite already completed exact scores. Bounds, missing/incomplete comparisons and depth below 10 remain **Uncertain**; increasing the review time/depth and restarting can provide more evidence. Defaults are depth 16 and at most 3000 ms per search; actual achieved depth is displayed with selected-move feedback. Pause retains completed progress locally; Resume continues it. A changed tree, engine, settings or review algorithm invalidates prior results; simply selecting another position does not.

**Brilliant** accepts best or near-best sacrifices at a common completed depth ≥12 with at most 50 centipawns of loss. The played continuation must retain an evaluation ≥−50 centipawns for the mover or a winning mate. Its own legal PV, inspected for up to six plies, must show the moved non-pawn piece being captured with a net material loss of at least two pawn units, followed by a reply that maintains that sacrifice or delivers checkmate. Forced moves and unsupported evidence do not qualify. This is an engine-backed heuristic, not proof. Other classification thresholds are unchanged. Review algorithm `chessin-review-v3` invalidates older stored grades; rerun review to apply the corrected evidence collection and more generous Brilliant criteria.

**ChessIn quality** is `round(mean(100 * exp(-min(lossCp,1000)/200)))` over completed graded non-forced moves per side, with coverage disclosed. It is neither Chess.com accuracy nor Elo.
