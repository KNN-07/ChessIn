...
Open **Engine settings** beneath the analysis sidebar heading, or use its gear, to open the full right-side drawer without leaving the board. The drawer includes engine profiles/downloads, search depth/time/nodes, supported hardware and line-count controls, remote connections and app settings. Escape, the close button or clicking outside closes it and restores keyboard focus; on phones it fills the screen. Configuration is disabled while game analysis runs; close the drawer and pause analysis before changing engines. Review depth/time remain under **Analysis settings & method**. The main navigation's Settings page remains available independently.
